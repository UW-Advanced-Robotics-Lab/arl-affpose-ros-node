...
class Transformer:
    ...
    def resize(self, image, target):
        ...
        scale_factor = int(min(self.min_size / min_size, self.max_size / max_size))
        size = [int(s * scale_factor) for s in ori_image_shape]
    ...
    def batched_image(self, image, stride=32):
        size = image.shape[-2:]
        max_size = tuple(int(math.ceil(s / stride) * stride) for s in size)
