...
% read keyframes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
keyframes = dir(fullfile(opt.eval_folder, '*.mat'));
fprintf('\nLoaded %d Keyframes \n', numel(keyframes))

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
num_preds = length(keyframes)*6; % ~6 objects per scene
...
errors_translation_x = zeros(num_preds, 1);
errors_translation_y = zeros(num_preds, 1);
errors_translation_z = zeros(num_preds, 1);
...
for i = 1:numel(keyframes)
    ...
    % load poses
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    filename = strcat(keyframes(i).folder, '/' , keyframes(i).name);
    poses = load(filename);
    disp(filename);
    ...
    % for each class
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for j = 1:numel(poses.class_ids)
        % class id         
        count = count + 1;
        cls_index = poses.class_ids(j);
